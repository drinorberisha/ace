...
export default function Navigation() {
  ...
  useEffect(() => {
    ...
    setIsLoggedIn(!!token); // Check if token exists on the client side
  ...
  return (
    <Disclosure as="nav" className="bg-gray-800 relative z-[99]">
      {({ open }) => (
        <>
                    ...
                    >
                      <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                        <Dropdown.Header>
                          ...
                          {/* Display user's name or "Guest" if not logged in */}
